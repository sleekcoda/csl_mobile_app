...
export class SellAndBuyStocksPage {
  ...
  constructor(private _storage: Storage,public _modalCtrl: ModalController, public navCtrl: NavController, public navParams: NavParams, private _spin: LoadingController , private _http: DashboardProvider, private _alert: NoticeHandlerProvider) {
    this.setupPortfolio();
    this.today    =  new Date();
    this.maxYear  =  this.today.getFullYear() + 5;
    this.minDate  =  this.today.getFullYear() +'-'+ (this.today.getMonth()+ 1) +'-'+ this.today.getDate();
    
    this.stockDetails.price       = this.currentPrice;
    this.stockDetails.expires     = this.minDate;

    this.actions                  = this.navParams.get('action');
  ...
  async setupPortfolio(){
    await this._storage.get('portfolio').then(
      portfolio => {
        ...
        if(this.actions == 'sell'){
          this.getPorfolioInvestments(this.portfolioArray[0].customerId, this.portfolioArray[0].marketAccountId);

        } // #if user want's to sell stock      
        if(this.actions == 'buy'){
          this.getStock(this.portfolioArray[0].customerId, this.portfolioArray[0].marketAccountId);
    ...
    this._http.getPorfolioInvestments(customerId,cscsNumber).then(
      response => {
        if(response.item1 == 200 ){
          ...
          } else{this._alert.notifyError(JSON.stringify('You have no investment'));}
  ...
  setAvailableStockQuantity(symbol: string){
    ...
    for (let i in this.availableStocks) {
  ...
  selectPorfolioInvestments(i: number){
    if(this.actions == 'sell'){
      ...
      this.getPorfolioInvestments(this.portfolioArray[i].customerId,this.portfolioArray[i].marketAccountId);
    } else {
      this.stockDetails.symbol = null;
      this.currentPrice = null;
      //Get the list of stocks available for users to trade
      this.getStock(this.portfolioArray[i].customerId,this.portfolioArray[i].marketAccountId);
    }
    this.cscsNumber = this.portfolioArray[i].marketAccountId;
    this.cashAccountStatement(this.portfolioArray[i].cashAccountId);
    
    this.stockDetails.portfolioDesc = this.portfolioArray[i].portfolioDescription;
  ...
  confirmOrder() {
    ...
    if(this.actions == 'buy'){ 
      ...
      this.stockDetails.reference   = "Buy " + this.stockDetails.symbol + '@'+ this.minDate ;
      this.navCtrl.push(ConfirmBuyOrderPage,this.stockDetails);

    } else if(this.actions == 'sell'){

      this.stockDetails.actions   = 'sell';
      this.stockDetails.reference = "Sell " + this.stockDetails.symbol + '@'+ this.minDate ;
  ...
  getStock(customerId,cscsNumber){
    let loading = this._spin.create({content:"Fetching available stocks'...",spinner:''});
    loading.present();

    let path = (this.actions == 'sell') ? `portfolios/${customerId}/investments/${cscsNumber}` : `stocks/${cscsNumber}`;
    
    this._http.plainGetRequest(path).then(
      response => {
        if(response.status == 'success'){
          ...
          this.today = new Date();
          this.maxYear = this.today.getFullYear() + 5;
          this.minDate = this.today.getFullYear().toString() +'-'+ (this.today.getMonth()+ 1).toString() +'-'+ this.today.getDate().toString();
          ...
          let selectedStock =  (this.preSelectedSymbol != null) ? this.preSelectedSymbol  : response.data[0].symbol;
    ...
    ).catch(reason => { this._alert.notifyError('Something went wrong');});
  }
    ...
    this._http.plainGetRequest(`stockprices/${symbol}`).then(
      response => {
        if(response.status == 'success'){
          ...
          this._alert.notifyError('Unable to retreive current stock price at the moment');
        }
      },
      error => {
        if(error.status == 401){
  ...
  validateDate(){
    setTimeout(()=>{
      ...
      let dateDifference  = new Date(this.stockDetails.expires).getTime() - today;
  ...
  cashAccountStatement(cashAccountId: string){
    ...
    this._http.plainGetRequest(`cashaccounts/${cashAccountId}/balance/`).then(
      response => {
        if(response.status == 'success'){
      ...
      error => {
        this.availableBalance = 0;
        if(error.status == '401'){this.navCtrl.setRoot(LoginPage);this._alert.notifyError('Session expired');}
